...
// =================================================================
// ╔═════════════════════════════════════════════════════════════╗
// ║                    KHỐI MÃ SPLASH SCREEN (PHUCUONQ)           ║
// ╚═════════════════════════════════════════════════════════════╝
#import <UIKit/UIKit.h>

// Định nghĩa lớp YTAppDelegate (đã xác nhận)
@interface YTAppDelegate : UIResponder <UIApplicationDelegate>
@end

// Định nghĩa Category để thêm phương thức của chúng ta
@implementation YTAppDelegate (PhuCuonqSplash)

// Phương thức để hiển thị Splash Screen
- (void)showPhuCuonqSplash {
    // 1. Tạo một cửa sổ (Window) mới
    UIWindow *splashWindow = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
    splashWindow.windowLevel = UIWindowLevelAlert + 1;
    splashWindow.backgroundColor = [UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.8f]; // Nền đen mờ

    // 2. Tạo Label để hiển thị chữ ký
    UILabel *label = [[UILabel alloc] init];
    label.text = @"MODDED BY PHUCUONQ"; // CHỮ KÝ CỦA BẠN
    label.textColor = [UIColor whiteColor];
    label.font = [UIFont boldSystemFontOfSize:24.0];
    label.textAlignment = NSTextAlignmentCenter;
    [label sizeToFit];
    
    // Canh giữa Label trong Window
    label.center = splashWindow.center;

    [splashWindow addSubview:label];
    splashWindow.alpha = 0.0; // Bắt đầu với độ trong suốt 0
    [splashWindow makeKeyAndVisible];

    // 3. Hoạt ảnh (Fade In - Hiện dần)
    [UIView animateWithDuration:0.5 animations:^{
        splashWindow.alpha = 1.0;
    } completion:^(BOOL finished) {
        // 4. Hoạt ảnh (Fade Out - Biến mất dần) sau 1.5 giây
...
// Activate FLEX và THÊM LOGIC SPLASH SCREEN
%hook YTAppDelegate
- (BOOL)application:(UIApplication *)application
	didFinishLaunchingWithOptions:(NSDictionary<UIApplicationLaunchOptionsKey, id> *)launchOptions {
	BOOL didFinishLaunching = %orig;

    // THÊM LOGIC SPLASH SCREEN VÀO ĐÂY
    [self performSelector:@selector(showPhuCuonqSplash) withObject:nil afterDelay:0.5];
